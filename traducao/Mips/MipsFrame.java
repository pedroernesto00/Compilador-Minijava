...
public class MipsFrame extends Frame {
    ...
    public Exp externalCall(String s, List<Exp> args) {
			...
			for (Exp arg : args) {
				explist = new ExpList(arg, explist);
			}
    ...
    public InstrList codegen(StmList stms) {
			InstrList insns = new InstrList(null, null);
			ArrayList<Instr> arraylist = new ArrayList<Instr>();
			Codegen cg = new Codegen(this);
			while (!(stms == null) && !(stms.head == null)) {
				insns = cg.codegen(stms.head);	
				while (insns != null) {
					arraylist.add(insns.head);
					System.out.print(insns.head.format(this));
					insns = insns.tail;
				}			
				stms = stms.tail;
			}		
			insns = new InstrList(null, null);
			InstrList aux = insns;			
			insns.head = arraylist.get(0);
			for (int i=1;i<arraylist.size();i++){
				InstrList instrlist = new InstrList(arraylist.get(i), null);
				insns.tail = instrlist;
				insns = insns.tail;
			}
			return aux;
    }
    ...
    public static Stm SEQ(Stm left, Stm right) {
	if (left == null)
	    return right;
	if (right == null)
	    return left;
